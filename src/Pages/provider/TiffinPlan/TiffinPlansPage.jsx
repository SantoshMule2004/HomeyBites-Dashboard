...
export default function TiffinPlansPage({ providerId: providerIdProp, readOnly = false, pageTitle }) {
  ...
  const providerId = providerIdProp ?? user?.userId;
    ...
    },
    {
      ...
      render: (plan) => (
        <div className="hb-table__desc">
          {plan.offersBreakfast && <div>B: { formatPrice(plan.pricePerBreakfast)}</div>}
          {plan.offersLunch && <div>L: {formatPrice(plan.pricePerLunch)}</div>}
          {plan.offersDinner && <div>D: {formatPrice(plan.pricePerDinner)}</div>}
        </div>
      ),
    },
    {
